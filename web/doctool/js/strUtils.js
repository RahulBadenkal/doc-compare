...
diff_match_patch.prototype.diff_linesToWords_ = function(text1, text2) {
  ...
  function diff_linesToCharsMunge_(text) {
    ...
    var textLength = text.length;  // It remains same throughout
    while (lineEnd < textLength - 1) {
      // Word Definition:
      // 1. Any blank space (chArr) is a word
      // 2. Any sequence of chars between two blank spaces (chArr) is a word
      // var ch_arr = [' ', '\n', '\t', '\b'];

      lineEnd = -1;
      for (let i=lineStart; i<textLength-1; i++){
        var letter = text[i];
        if (letter === ' ' || letter === '\n' || letter === '\t' || letter === '\b'){
          lineEnd = i;
          break;
      ...
      if (lineHash.hasOwnProperty ? lineHash.hasOwnProperty(line) :
        ...
        if (lineArrayLength === maxLines) {
          // Bail out at 65535 because
          // String.fromCharCode(65536) == String.fromCharCode(0)
          console.log("Diff unicode limit reached while doing word diff");
          line = text.substring(lineStart);
          lineEnd = textLength;
        }
        chars += String.fromCharCode(lineArrayLength);
  ...
  // Allocate 2/3rds of the space for text1, the rest for text2.
  var maxLines = 40000;  // The code will not work properly when the number of unique words in text1 more than this
  var chars1 = diff_linesToCharsMunge_(text1);
  maxLines = 65535;  // The code will not work properly when the number of unique words in CONCATENATE(text1, text2) is more than this
  var chars2 = diff_linesToCharsMunge_(text2);
  return {chars1: chars1, chars2: chars2, lineArray: lineArray};
};
...
function getTextDiffs(text1, text2, diff_mode = 0, timeout = 9.5) {
