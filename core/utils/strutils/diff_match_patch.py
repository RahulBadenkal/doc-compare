...
class diff_match_patch:
  ...
  def __init__(self):
    """Inits a strUtils object with default settings.
      ...
      while lineEnd < len(text) - 1:
        ...
        if line in lineHash:
          ...
          if len(lineArray) == maxLines:
            ...
            logging.error("Diff match patch Problem. No. of Unique Lines exceeds the max limit")
            line = text[lineStart:]
            lineEnd = len(text)
  ...
  def diff_linesToWords(self, text1, text2):
    ...
    def diff_linesToCharsMunge(text):
      ...
      while lineEnd < len(text) - 1:
        ...
        lineEnd = -1
        for pos in range(lineStart, len(text)):
          if text[pos] in chArr:
            lineEnd = pos
            break
        ...
        if line in lineHash:
          ...
          if len(lineArray) == maxLines:
            # Bail out at 1114111 because chr(1114112) throws.
            logging.error("Diff match patch Problem. No. of Unique Words exceeds the max limit")
            line = text[lineStart:]
            lineEnd = len(text)
          lineArray.append(line)
          lineHash[line] = len(lineArray) - 1
